...
放课瞬间服务端会进入一段「系统正在初始化」的状态。实测这一次持续了**约 3 分钟**
（放课后 9 分钟时还有一次会话复验失败）。这段时间的现象：
...
**踩过的坑**：脚本里有个"先读容量、没空位就不发写请求"的优化（为了挂机捡漏时
不浪费写额度）。初始化期间容量返回 `0/0`，被算成 `0-0>0 = False`（已满），
于是脚本在**整个放课窗口里安静地轮询了 56 秒**，一发写请求都没发出去。

两处修正：

1. `has_slot()` 把 `total <= 0` 当成"查不到"（返回 `None` → 照打），不再当成"满"。
2. 服务端初始化/限流期间，**停机时间不计入"第一组多少秒没名额就换组"的倒计时**，
   并且把试探节奏改成「每 0.6 秒 1 发」（多打无意义，但必须快速试探 ——
   服务恢复的那一瞬间才是位子真正可抢的时刻）。

回归测试：`tests/test_offline.py::test_init_outage_keeps_firing` 用一个
"容量全返回 0/0 + 写请求全被拒"的假服务端驱动**真实的重试循环**，断言停机期间
仍在发写请求。把修复退回旧逻辑，这个测试会立刻变红（实测 0 发 vs 6 发）。
